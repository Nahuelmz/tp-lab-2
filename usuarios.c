...
int validarUserExiste(char archiUsu[], char nombreUsuario[]) // Valida si el nombre del usuario que se quiere dar de alta ya existe en la base
{
...
int validarUsuExisteNombre(char nombre[]){
...
int validarUsuExisteArrayId(int id) // Valida si el id ingresado existe en el arreglo de usuarios activos(que igualmente contiene a los inactivos con su campo eliminados en 1 para que puedan restaurarse)
{
...
void altaUsuarios(char archiUsu[])// Funcion general de alta de usuarios
{
    ...
    else // Si se pudo abrir el archivo, ejecuta el ingreso de tantos usuarios como la persona desee(var de control)
    {
        ...
        while(control=='s')
        {
            printf("Ingrese un nombre de usuario Alfanumerico o alfabetico valido: ");
            fflush(stdin);
            gets(usuAux.nombreUsuario);

            while (!Exito && i<4)
            {
                if(validarUserExiste(archiUsu, usuAux.nombreUsuario)) // Chequea que el nombre de usuario en ingreso no exista en el archivo para evitar que se duplique mediante un flag
                {
    ...
    void bajaUsuario(char archiUsu[]) //Elimina usuarios pasando asignando valor verdadero al campo "eliminado"
    {
        char control='n';
        int idUsu=0;
        int posId=0;
        ...
        else
        {
            ...
            if(validarUsuExisteArrayId(idUsu) && validarUsuActivo(idUsu))
            {
...
/// OJO ver con Nahuel por mostrar Nodo Pelis
    void mostrarUsuariosActivos() //TP 2: Pasa a la función imprimir usuarios los usuarios activos para mostrar sus datos
    {
        int i=0;
        system("cls");
...
//Se usa para la modificación del registro, cuando se puede revertir la condición de eliminado. NO se muestra el pass al administrador
    ...
    int buscarPosicionUsuario(char nombreUsuario[]) // Busca la posicion del usuario en el arreglo por nombre de usuario
    {
    ...
    int buscarPosUsuarioPorId(int id) // Busca la posicion del usuario en el arreglo por nombre de usuario
    {
        int i=0;
    ...
    void consultarUsuario()
    {
        ...
        if(opcion==2)
        {
            ...
            fflush(stdin);
            gets(nombreAux);
            pos=buscarPosicionUsuario(nombreAux);
            if(arregloUsuActivos[pos].usr.eliminado==0)
            {
                printf("Los datos del usuario consultado son: \n");
                imprimirUsuarioSinPass(arregloUsuActivos[pos].usr);
    ...
    int modificarUsuario(char archiUsu[])//Modificacion de usuario para administrador - NO se muestra el password
    {
        int id;
        ...
        scanf("%i", &id);
        system("cls");
        if(validarUsuExisteArrayId(id)) // Avanza en la modificación en caso que el usuario exista
        {

            printf("\nLos datos actuales del usuario son:\n");
            int pos=buscarPosUsuarioPorId(id); // Busca la posición del usuario en el arreglo
    ...
    int cambiarNombreUsu(char archiUsu[], int id, int pos)
    {
        int flag=0;
        stUsuario usuAux;
    ...
    int cambiarPassUsu(char archiUsu[], int id, int pos) /// VER CON NAHUEL
    {
        int flag=0;
        stUsuario usuAux;
...
//        encriptacionPass(passAux, arregloUsuActivos[posId].usr.pass); /// Ver con NAHUEL -- Actualiza en arreglo?

            fwrite(&usuAux, sizeof(usuAux), 1, archi); // Se actualiza en archivo

//        strcpy(arregloUsuActivos[pos].usr.pass, usuAux.pass); // Actualiza en arreglo
    ...
    int cambiarAnioUsu(char archiUsu[], int id, int pos)
    {
        int flag=0;
        stUsuario usuAux;
    ...
    int cambiarGeneroUsu(char archiUsu[], int id, int pos)
    {
        int flag=0;
        stUsuario usuAux;
    ...
    int cambiarPaisUsu(char archiUsu[], int id, int pos)
    {
        int flag=0;
        stUsuario usuAux;
    ...
    int cambiarEstadoUsu(char archiUsu[], int id, int pos)
    {
        int flag=0;
        stUsuario usuAux;
    ...
    int cambiarPermisosUsu(char archiUsu[], int id, int pos)
    {
        int flag=0;
        stUsuario usuAux;
